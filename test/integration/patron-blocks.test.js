...
export default test('patron blocks section', { permissions: ['manualblocks.collection.get'] })
  ...
  .assertion(Div({ ariaLabelledBy: 'accordion-toggle-button-patronBlocksSection' })
    .find(TableRowGroup())
  ...
  //   .step(TableRow({ rowNumber: 1 }).find(TableCell('Manual')).click())
  //   // 🧹 clicking on block does not direct user to page with information
  //   .step(TextArea({ name: 'desc' }).fill('Mail sent to patron has bounced back.'))
  //   .step(Button('Save & Close').click())
  //   .assertion(TableRow({ rowNumber: 1 }).find(TableCell('Mail sent to patron has bounced back.')).exists())
  //   .assertion(Div({ ariaLabelledBy: 'accordion-toggle-button-patronBlocksSection' })
  //     .find(TableRowGroup())
  //     .has({ dataRowContainerCount: 3 }))
  // )
