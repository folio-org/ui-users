...
export default test('settings')
  ...
  .child('fee/fines manual/charges', test => test
    ...
    .child('delete charge', test => test
      .step(TableRow({ dataRowIndex: 'row-0' }).find(Button({ ariaLabel: 'Delete this item' })).click())
      ...
      .child('confirm deletion', test => test
        ...
        .step(Div({ attribute: 'data-test-callout-element' }).find(Div('The Fee/Fine Type Feefine 1 was successfully deleted')).exists())
        .assertion(TableRow().absent())))
    .child('edit change', test => test
      .step(TableRow({ dataRowIndex: 'row-0' }).find(Button({ ariaLabel: 'Edit this item' })).click())
      .step(TextField({ placeholder: 'defaultAmount' }).fill('300'))
      .step(Select({ name: 'items[0].actionNoticeId' }).select('Template 1'))
      .child('cancel edit', test => test
        .step(Button('Cancel').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('1050.00')).exists()))
      .child('confirm edit', test => test
        .step(Button('Save').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('300.00')).exists())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Template 1')).exists())))
    .child('create new', test => test
      .step(Button({ id: 'clickable-add-settings-feefines' }).click())
      .step(TextField({ placeholder: 'feeFineType' }).fill('New feefine'))
      .step(TextField({ placeholder: 'defaultAmount' }).fill('100'))
      .step(Select({ name: 'items[0].chargeNoticeId' }).select('Template 3'))
      .step(Button('Save').click())
      .assertion(TableRowGroup().has({ dataRowContainerCount: 2 }))
      .assertion(TableRow({ dataRowIndex: 'row-1' }).find(TableCell('New feefine')).exists())
      .assertion(TableRow({ dataRowIndex: 'row-1' }).find(TableCell('100.00')).exists())
      .assertion(TableRow({ dataRowIndex: 'row-1' }).find(TableCell('Template 3')).exists()))
    .child('create new with error', test => test
      .step(Button({ id: 'clickable-add-settings-feefines' }).click())
      .child('fee/fine type when empty', step => step
        .step(TextField({ placeholder: 'feeFineType' }).fill(''))
      ...
      .child('duplicate fee/fine type', step => step
        .step(TextField({ placeholder: 'feeFineType' }).fill('Damage camera fee0'))
        .assertion(Button('Save', { disabled: true }).exists())
        .assertion(Alert('Fee/fine type exists for owner Main Admin3').exists()))
      .child('amount is non numeric', test => test
        .step(TextField({ placeholder: 'defaultAmount' }).fill('hundred'))
        .assertion(Button('Save', { disabled: true }).exists())
    ...
    .child('owner charge notice', test => test
      .step(Select({ id: 'select-owner' }).select('Main Admin3'))
      .step(Button('Edit').click())
      .step(Select({ name: 'defaultChargeNoticeId' }).select('Template 3'))
      .step(Select({ name: 'defaultActionNoticeId' }).select('Template 2'))
      .child('cancel edit', test => test
        .step(Button({ id: 'charge-notice-cancel' }).click())
        .assertion(Div({ id: 'defaultChargeNoticeId', value: '-' }).exists())
        .assertion(Div({ id: 'defaultActionNoticeId', value: '-' }).exists()))
      .child('save edit', test => test
        .step(Button('Save').click())
        .assertion(Div({ id: 'defaultChargeNoticeId', value: 'Template 3' }).exists())
        .assertion(Div({ id: 'defaultActionNoticeId', value: 'Template 2' }).exists())))
    .child('copy feefines to another owner', test => test
      .step(Select({ id: 'select-owner' }).select('Main Admin1'))
      .step(Select({ name: 'ownerId' }).select('Main Admin3'))
  ...
  .child('fee/fine owners', test => test
    ...
    .child('delete', test => test
      .step(TableRow({ dataRowIndex: 'row-0' }).find(Button({ id: 'clickable-delete-settings-owners-0' })).click())
    ...
    .child('edit', test => test
      .step(TableRow({ dataRowIndex: 'row-0' }).find(Button({ id: 'clickable-edit-settings-owners-0' })).click())
      .step(TextField({ placeholder: 'owner' }).fill('Main Admin10'))
      .step(TextField({ placeholder: 'desc' }).fill('Owner Test'))
      .child('cancel edit', test => test
        .step(Button('Cancel').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' })
      ...
      .child('save edit', test => test
        .step(Button('Save').click())
        .assertion(TableRow({ dataRowIndex: 'row-1' })
    ...
    .child('adding owner', test => test
      .step(Button({ id: 'clickable-add-settings-owners' }).click())
      .child('submit new owner', test => test
        .step(TextField({ placeholder: 'owner' }).fill('Main CUIB'))
        .step(TextField({ placeholder: 'desc' }).fill('CUIB'))
        .step(Button('Save').click())
        .assertion(TableRow({ dataRowIndex: 'row-4' })
      ...
      .child('type pre-existing owner', test => test
        .step(TextField({ placeholder: 'owner' }).fill('Main Admin0'))
    ...
    .child('edit service-point', test => test
      ...
      .step(TextField({ placeholder: 'owner' }).fill('Main Admin10'))
      .step(TextField({ placeholder: 'desc' }).fill('Owner Test'))
      .step(Button({ ariaLabel: 'open menu' }).click())
      .step(ListItem('owner-service-point-main-item-0').click())
      .step(Button('Save').click())
      .assertion(Div({ ariaLabelledBy: 'owner-service-point-label' }).absent())))
  .child('fee/fine payment methods', test => test
    ...
    .child('delete', test => test
      .step(Button({ id: 'clickable-delete-settings-payments-0' }).click())
      .child('cancel delete', test => test
        .step(Button('Cancel').click())
        .assertion(TableRowGroup().has({ dataRowContainerCount: 5 })))
      .child('confirm delete', test => test
        ...
        .assertion(TableRowGroup().has({ dataRowContainerCount: 4 }))))
    .child('edit', test => test
      .step(Button({ id: 'clickable-edit-settings-payments-0' }).click())
      .step(TextField({ placeholder: 'nameMethod' }).fill('Cash10'))
      .step(Select({ name: 'items[0].allowedRefundMethod' }).select('Yes'))
      .child('cancel edit', test => test
        .step(Button('Cancel').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Cash0')).exists())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('No')).exists()))
      .child('confirm edit', test => test
        .step(Button('Save').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Cash10')).exists())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Yes')).exists())))
    .child('add a payment', test => test
      .step(Button({ id: 'clickable-add-settings-payments' }).click())
      .step(TextField({ placeholder: 'nameMethod' }).fill('Cash10'))
      .step(Select({ name: 'items[0].allowedRefundMethod' }).select('Yes'))
      .step(Button('Save').click())
      .assertion(TableRow({ dataRowIndex: 'row-5' }).find(TableCell('Cash10')).exists())
      .assertion(TableRow({ dataRowIndex: 'row-5' }).find(TableCell('Yes')).exists()))
    .child('add a pre-existing payment', test => test
      .step(Button({ id: 'clickable-add-settings-payments' }).click())
      .step(TextField({ placeholder: 'nameMethod' }).fill('Cash2'))
  ...
  .child('fee/fine refund reasons', test => test
    ...
    .child('delete', test => test
      .step(Button({ id: 'clickable-delete-settings-refunds-0' }).click())
      .child('cancel delete', test => test
        .step(Button('Cancel').click())
        .assertion(TableRowGroup().has({ dataRowContainerCount: 5 })))
      .child('confirm delete', test => test
        ...
        .assertion(TableRowGroup().has({ dataRowContainerCount: 4 }))))
    .child('edit', test => test
      .step(Button({ id: 'clickable-edit-settings-refunds-0' }).click())
      .step(TextField({ placeholder: 'nameReason' }).fill('Reason10'))
      .step(TextField({ placeholder: 'description' }).fill('Reason Desc10'))
      .child('cancel edit', test => test
        .step(Button('Cancel').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Reason0')).exists())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Reason Desc0')).exists()))
      .child('confirm edit', test => test
        .step(Button('Save').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Reason10')).exists())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Reason Desc10')).exists())))
    .child('add a refund', test => test
      .step(Button({ id: 'clickable-add-settings-refunds' }).click())
      .step(TextField({ placeholder: 'nameReason' }).fill('Reason10'))
      .step(TextField({ placeholder: 'description' }).fill('Reason Desc10'))
      .step(Button('Save').click())
      .assertion(TableRow({ dataRowIndex: 'row-5' }).find(TableCell('Reason10')).exists())
      .assertion(TableRow({ dataRowIndex: 'row-5' }).find(TableCell('Reason Desc10')).exists()))
    .child('add a pre-existing refund', test => test
      .step(Button({ id: 'clickable-add-settings-refunds' }).click())
      .step(TextField({ placeholder: 'nameReason' }).fill('Reason1'))
      .step(Button('Save').click())
  ...
  .child('fee/fine transfer accounts', test => test
    ...
    .child('delete', test => test
      .step(Button({ id: 'clickable-delete-settings-transfers-0' }).click())
      .child('cancel delete', test => test
        .step(Button('Cancel').click())
        .assertion(TableRowGroup().has({ dataRowContainerCount: 2 })))
      .child('confirm delete', test => test
        ...
        .assertion(TableRowGroup().has({ dataRowContainerCount: 1 }))))
    .child('edit', test => test
      .step(Button({ id: 'clickable-edit-settings-transfers-0' }).click())
      .step(TextField({ placeholder: 'accountName' }).fill('USA Bank3'))
      .step(TextField({ placeholder: 'desc' }).fill('Transfer place3'))
      .child('cancel edit', test => test
        .step(Button('Cancel').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('USA Bank0')).exists())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Transfer place0')).exists()))
      .child('confirm edit', test => test
        .step(Button('Save').click())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('USA Bank3')).exists())
        .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('Transfer place3')).exists())))
    .child('add new transfer', test => test
      .step(Button({ id: 'clickable-add-settings-transfers' }).click())
      .step(TextField({ placeholder: 'accountName' }).fill('USA Bank5'))
      .step(TextField({ placeholder: 'desc' }).fill('Transfer place5'))
      ...
      .child('confirm addition', test => test
        .step(Button('Save').click())
        .assertion(TableRow({ dataRowIndex: 'row-2' }).find(TableCell('USA Bank5')).exists())
        .assertion(TableRow({ dataRowIndex: 'row-2' }).find(TableCell('Transfer place5')).exists()))))
  .child('fee/fine waive reasons', test => test
    ...
    .child('edit and cancel', test => test
      .step(TableRow({ dataRowIndex: 'row-0' }).find(Button({ ariaLabel: 'Edit this item' })).click())
      .step(TextField({ placeholder: 'nameReason' }).fill('First time offender99'))
      .step(Button('Cancel').click())
      .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('First time offender0')).exists()))
    .child('edit and save', test => test
      .step(TableRow({ dataRowIndex: 'row-0' }).find(Button({ ariaLabel: 'Edit this item' })).click())
      .step(TextField({ placeholder: 'nameReason' }).fill('First time offender99'))
      .step(Button('Save').click())
      .assertion(TableRow({ dataRowIndex: 'row-0' }).find(TableCell('First time offender99')).exists()))
    .child('add a waive', test => test
      .step(Button({ id: 'clickable-add-settings-waives' }).click())
      .step(TextField({ placeholder: 'nameReason' }).fill('First time offender10'))
      .step(TextField({ placeholder: 'description' }).fill('Penalty abatement10'))
      .step(Button('Save').click())
      .assertion(TableRow({ dataRowIndex: 'row-5' }).find(TableCell('First time offender10')).exists())
      .assertion(TableRow({ dataRowIndex: 'row-5' }).find(TableCell('Penalty abatement10')).exists()))
    .child('add a pre-existing waive', test => test
      .step(Button({ id: 'clickable-add-settings-waives' }).click())
      .step(TextField({ placeholder: 'nameReason' }).fill('First time offender1'))
      .step(Button('Save').click())
  ...
  .child('general custom fields', test => test
    ...
    .child('edit permission', test => test
      ...
      .assertion(Div({ id: 'custom-fields-list', sectionsCount: 4 }).exists())
      .child('edit', test => test
        .step(Link('Edit').click())
        .assertion(Header('Edit custom fields').exists())
        .child('has delete buttons', test => test
          .assertion(Section({ attribute: 'data-test-accordion-section', id: '1' })
            .find(Button({ attribute: 'data-test-custom-field-delete-button' })).exists())
          .assertion(Section({ attribute: 'data-test-accordion-section', id: '2' })
            .find(Button({ attribute: 'data-test-custom-field-delete-button' })).exists())
          .assertion(Section({ attribute: 'data-test-accordion-section', id: '3' })
            .find(Button({ attribute: 'data-test-custom-field-delete-button' })).exists())
          .assertion(Section({ attribute: 'data-test-accordion-section', id: '4' })
            .find(Button({ attribute: 'data-test-custom-field-delete-button' })).exists())))))
  .child('general departments', test => test
    .step(Link('Departments').click())
    .assertion(TableRowGroup().has({ dataRowContainerCount: 2 }))
    .assertion(TableRow({ dataRowIndex: 'row-1' })
      .find(Button({ ariaLabel: 'Delete this item' }))
      .absent())
    .child('create new department errors', test => test
      .step(Button({ id: 'clickable-add-departments' }).click())
      .child('pre-existing name', child => child
        .step(TextField({ placeholder: 'name' }).fill('Test1'))
        .assertion(Alert('Name already exists').exists()))
      .child('pre-existing code', child => child
        .step(TextField({ placeholder: 'code' }).fill('test1'))
        .assertion(Alert('Code already exists').exists()))
      .child('empty fields', child => child
        .step(TextField({ placeholder: 'name' }).fill(''))
        .step(TextField({ placeholder: 'code' }).fill(''))
        .assertion(Alert('Please fill this in to continue').exists())
  ...
  .child('fee/fine comment required', test => test
    ...
    .assertion(Form({ id: 'form-require-comment' })
      .find(Div({ id: 'paid' }))
