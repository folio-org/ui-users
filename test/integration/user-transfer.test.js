...
export default test('Transfer user fines', {
  ...
  .child('open accounts', test => test
    ...
    .child('transfer prompt', test => test
      ...
      .step(Button({ id: 'open-closed-all-transfer-button' }).click())
      .child('cancel the transfer', test => test
        .step(Button('Cancel').click())
        .assertion(Div({ id: 'transfer-modal' }).absent()))
      .child('add a transfer', test => test
        .step(TextField({ id: 'amount' }).fill('200.00'))
        .step(Select({ name: 'method' }).select('USA Bank1'))
        .step(TextArea({ name: 'comment' }).fill('Comment UNAM'))
        .step(Div({ id: 'transfer-modal' }).find(Button('Transfer')).click())
  ...
  .child('all accounts', test => test
    ...
    .child('deselect closed account and continue transfer', test => test
      .step(Table('warning-mcl').find(TableRow({ rowNumber: 1 })).find(Checkbox()).click())
      .step(Button({ id: 'warningTransferContinue' }).click())
      .step(TextField({ id: 'amount' }).fill('400.00'))
      .step(Select({ name: 'method' }).select('USA Bank1'))
      .step(TextArea({ name: 'comment' }).fill('Comment UNAM'))
      .step(Button({ id: 'submit-button' }).click())
      .step(Button('Confirm').click())
      .assertion(Table('list-accounts-history-view-feesfines', { dataRowCount: 5 }).exists())
      .assertion(Table('list-accounts-history-view-feesfines')
        .find(TableRowGroup())
        .find(TableRow({ rowNumber: 1 }))
        .find(TableCell('0.00', { columnTitle: 'Remaining' }))
        .exists())
      .assertion(Table('list-accounts-history-view-feesfines')
        .find(TableRowGroup())
        .find(TableRow({ rowNumber: 2 }))
        .find(TableCell('10.00', { columnTitle: 'Remaining' }))
        .exists())
      .assertion(Table('list-accounts-history-view-feesfines')
        .find(TableRowGroup())
        .find(TableRow({ rowNumber: 3 }))
        .find(TableCell('20.00', { columnTitle: 'Remaining' }))
        .exists())
      .assertion(Table('list-accounts-history-view-feesfines')
        .find(TableRowGroup())
        .find(TableRow({ rowNumber: 4 }))
        .find(TableCell('30.00', { columnTitle: 'Remaining' }))
        .exists())
      .assertion(Table('list-accounts-history-view-feesfines')
        .find(TableRowGroup())
        .find(TableRow({ rowNumber: 5 }))
        .find(TableCell('0.00', { columnTitle: 'Remaining' }))
        .exists())));
