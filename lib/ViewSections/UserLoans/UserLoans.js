    ...
    return (
      ...
      >
        {loansLoaded ?
          <List
            listStyle="bullets"
            itemFormatter={(item) => (<li><a id={item.id} onClick={item.onClick}>{ item.label } <FormattedMessage id={item.formattedMessageId} /></a></li>)}
            items={[
              {
                id: 'clickable-viewcurrentloans',
                onClick: this.props.onClickViewOpenLoans,
                label: openLoansCount,
                formattedMessageId: 'ui-users.loans.openLoans',
              },
              {
                id: 'clickable-viewclosedloans',
                onClick: this.props.onClickViewClosedLoans,
                label: closedLoansCount,
                formattedMessageId: 'ui-users.loans.closedLoans',
              },
            ]}
