...
const ExtendedInfo = ({ expanded, onToggle, accordionId, stripes, user }) => (
  ...
  >
    <Row>
      <Col xs={3}>
        <KeyValue label="Date enrolled" value={formatDate(_.get(user, ['enrollmentDate'], ''), stripes.intl)} />
      </Col>
      <Col xs={3}>
        <KeyValue label="External system ID" value={_.get(user, ['externalSystemId'], '')} />
      </Col>
      <Col xs={3}>
        <KeyValue label="Birth date" value={formatDate(_.get(user, ['personal', 'dateOfBirth'], ''), stripes.intl)} />
      </Col>
      <Col xs={3}>
