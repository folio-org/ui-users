...
class MultiColumnListUsers extends React.Component{
  ...
  render(){
    const { contentData, onClickItemHandler, onSortHandler, sortHeading } = this.props;
      ...
      contentData.map(function(item, i){
        ...
        for(var prop in item){
          if (prop !== "id") {
            ...
            let val = item[prop];
            if (val === true) val = '✓';
            listItem.push(<td key={prop}>{val}</td>);
          }
        }

        listItems.push(<tr key={i} onClick={()=>onClickItemHandler(item.id)}>{listItem}</tr>);
        headers.map((header, i)=> headerCells.push(<th key={i} onClick={()=>onSortHandler(header)} style={header !== sortHeading ? {} : { 'textDecoration': 'underline' }}>{header}</th>), this);
