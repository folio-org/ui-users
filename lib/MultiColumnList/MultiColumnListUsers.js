...
class MultiColumnListUsers extends React.Component{
  ...
  render(){
    const { contentData, onClickItemHandler, onSortHandler } = this.props;
    ...
    if(contentData){
      contentData.map(function(item, i){
        ...
        headers.map((header, i)=> headerCells.push(<th key={i} onClick={()=>onSortHandler(header)}>{header}</th>), this);
