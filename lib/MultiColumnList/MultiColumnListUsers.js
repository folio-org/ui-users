...
class MultiColumnListUsers extends React.Component{
  ...
  render(){
    const { contentData, onClickItemHandler, onSortHandler, sortOrder } = this.props;
    ...
    if(contentData){
      contentData.map(function(item, i){
        ...
        headers.map((header, i)=> headerCells.push(<th key={i} onClick={()=>onSortHandler(header)} style={header !== sortOrder ? {} : { 'textDecoration': 'underline' }}>{header}</th>), this);
