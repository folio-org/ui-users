...
class Charge extends React.Component {
  ...
  onClickCharge(type) {
    ...
    const item = {
      ...
      materialTypeId: (this.item.materialType || {}).id || '0',
    };
    ...
    if (!selectedLoan.id) {
      ...
      type.title = selectedLoan.item.title;
      type.barcode = selectedLoan.item.barcode;
      type.callNumber = selectedLoan.item.callNumber;
      type.location = selectedLoan.item.location.name;
      type.materialType = selectedLoan.item.materialType.name;
    ...
    action.createdAt = type.feeFineOwner;
    if (type.comments) {
        ...
        <PayModal
          ...
          onSubmit={(values) => {
            ...
            if (this.type.remaining === '0.00') {
              ...
              this.type.status.name = 'Open';
            }
            this.onClickCharge(this.type);
          }}
        />
      </div>
    );
