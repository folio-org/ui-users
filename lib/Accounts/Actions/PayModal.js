...
class PayModal extends React.Component {
  ...
  render() {
    ...
    const message = `${(this.state.amount < selected) ? 'Partially paying' : 'Paying'} ${n} ${(n === 1) ? 'fee/fine' : 'fees/fines'} for a total amount of ${parseFloat(selected).toFixed(2)}`;
    ...
    return (
      ...
      >
        <form>
          ...
          <br /><br />
          <Row>
            <Col xs><FormattedMessage id="ui-users.accounts.pay.field.comment" />{(this.props.commentRequired) ? '*' : ''}</Col>
          </Row>
          <br />
          <Row>
            <Col xs>
