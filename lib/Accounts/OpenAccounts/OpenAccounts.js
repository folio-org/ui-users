...
const sortMap = {
  'Instance(Item Type)': f => f.title,
  'Charged Amount': f => f.amount,
  'Remaining Amount': f => f.remaining,
  'barcode': f => f.barcode,
  'date created': f => (f.metadata || {}).createdDate,
  'date updated': f => (f.metadata || {}).updatedDate,
...
class OpenAccounts extends React.Component {
  static manifest = Object.freeze({
    comments: {
      ...
      path: 'feefineactions?query=userId=%{activeRecord.id}&limit=200',
    },
    activeRecord: {},
  });
