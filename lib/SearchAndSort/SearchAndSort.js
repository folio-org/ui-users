...
class SearchAndSort extends React.Component {
  ...
  constructor(props) {
    ...
    const match = this.props.path.match('/users/view/');
  ...
  onSelectRow = this.props.onSelectRow ? this.props.onSelectRow : (e, meta) => {
    const id = meta.id;
    this.log('action', `clicked ${id}, selected record =`, meta);
    this.setState({ selectedItem: meta });
    this.props.history.push(`/users/view/${id}${this.props.location.search}`);
  }
  ...
  getRowURL(id) {
    return `/users/view/${id}${this.props.location.search}`;
  }
  ...
  render() {
    const { parentResources, stripes, urlQuery, filterConfig, moduleName, newRecordPerms, viewRecordPerms, objectName } = this.props;
