...
class SearchAndSort extends React.Component {
  ...
  constructor(props) {
    ...
    let queryResource = props.parentResources.query || {};
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(this.state.filters.uninitialized && this.props.parentResources.query) {
      this.setState({ filters: initialFilterState(this.props.filterConfig, this.props.parentResources.query.filters) });
    }
  ...
  onChangeSearch = (e) => {
    ...
    this.props.parentMutator.userCount.replace(this.props.initialResultCount);
  ...
  performSearch = _.debounce((query) => {
    ...
    this.props.parentMutator.query.update({ search: query });
  }, 350);
  ...
  onSort = (e, meta) => {
    ...
    this.props.parentMutator.query.update({ sort: sortOrder });
  }
  ...
  onNeedMore = () => {
    this.props.parentMutator.userCount.replace(this.props.parentResources.userCount + this.props.resultCountIncrement);
  }
  ...
  updateFilters = (filters) => { // provided for onChangeFilter
    ...
    this.props.parentMutator.query.update({ filters: Object.keys(filters).filter(key => filters[key]).join(',') });
  }
