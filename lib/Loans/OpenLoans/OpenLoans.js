...
class OpenLoans extends React.Component {
  ...
  constructor(props) {
    ...
    this.getEffectiveLocations = this.getEffectiveLocations.bind(this);
    ...
    this.possibleColumns = ['  ', 'title', 'itemStatus', 'dueDate', 'requests', 'barcode', 'Call number',
    ...
    this.controllableColumns = ['title', 'itemStatus', 'dueDate', 'requests', 'barcode', 'Call number',
      'Contributors', 'renewals', 'loanPolicy', 'location', 'loanDate'];
    ...
    this.sortMap = {
      ...
      [stripes.intl.formatMessage({ id: 'ui-users.loans.details.location' })]: loan => {
        const barcode = _.get(loan, ['item', 'barcode'], '');
        return this.state.effectiveLocations[barcode] || '';
      },
    };
  ...
  getEffectiveLocations() {
    const query = this.state.loans.map(loan => {
      return `barcode==${loan.item.barcode}`;
    }).join(' or ');
    this.props.mutator.items.reset();
    this.props.mutator.items.GET({ params: { query } }).then((items) => {
      const effectiveLocations = items.reduce((map, item) => {
        map[item.barcode] = item.effectiveLocation.name || '';
        return map;
      }, {});
      this.setState({ effectiveLocations });
    });
  }
