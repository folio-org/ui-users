...
class OpenLoans extends React.Component {
  ...
  showSingleRenewCallout(loan) {
    const verb = <strong>{this.props.stripes.intl.formatMessage({ id: 'ui-users.loans.item.renewed.calloutVerb' })}</strong>;
    const message = (
      <span>
        <FormattedMessage
      ...
      <p>
        <FormattedMessage
          ...
          values={{
            strongCount: <strong>{loanItems.length}</strong>,
            count: loanItems.length,
            verb: <strong>{this.props.stripes.intl.formatMessage({ id: 'ui-users.loans.item.renewed.calloutVerb' })}</strong>,
          }}
        />
      </p>
    );
