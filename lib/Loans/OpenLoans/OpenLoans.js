...
class OpenLoans extends React.Component {
  ...
  renew(loan, patron, bulkRenewal) {
    const promise = this.props.renew(loan, patron, bulkRenewal);
  ...
  renewSelected() {
    ...
    const renewSuccess = [];
    const renewFailure = [];
    const patron = this.props.user;

    const renewedLoans = selectedLoans.map((loan) => {
      const prom = (selectedLoans.length > 1) ? this.renew(loan, patron, true) : this.renew(loan, patron, false);
      return prom
