...
class OpenLoans extends React.Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (prevState.loans.length < nextProps.loans.length) {
      if (!prevState.loanPolicies) {
        return { loans };
      } else {
        return {
          loans,
          loanPolicies: prevState.loanPolicies
        };
      }
    }
    return null;
  ...
  constructor(props) {
    ...
    this.fetchLoanPolicy = this.fetchLoanPolicy.bind(this);
  ...
  fetchLoanPolicyNames() {
    ...
    this.props.mutator.loanPolicies.GET({ params: { query } }).then((loanPolicies) => {
      let loanPolicyObject = this.state.loanPolicies;
      loanPolicyObject = loanPolicies.map(loanPolicy => {
        if (!Object.prototype.hasOwnProperty.call(this.state.loanPolicies, loanPolicy.id)) {
          const policy = {};
          policy[loanPolicy.id] = loanPolicy.name;
          return { ...loanPolicyObject, ...policy };
        }
        return loanPolicyObject;
      });
  ...
  render() {
    ...
    const visibleColumns = ['  ', 'title', 'itemStatus', 'dueDate', 'requestQueue', 'barcode', 'feeFine', 'Call number', 'Contributors', 'renewals', 'loanPolicy', 'location', 'loanDate', ' '];
    const columnMapping = {
      ...
      'requestQueue': this.props.stripes.intl.formatMessage({ id: 'ui-users.loans.details.requestQueue' }),
      'feeFine': this.props.stripes.intl.formatMessage({ id: 'ui-users.loans.details.feeFine' }),
    ...
    return (
      <div className={css.root}>
        ...
        <MultiColumnList
          ...
          columnWidths={{ '  ': 28, 'title': 150, 'itemStatus': 100, 'dueDate': 130, 'requestQueue': 100, 'barcode': 120, 'feeFine': 80, 'Call number': 110, 'Contributors': 160, 'renewals': 70, 'loanPolicy': 100, 'location': 100, 'loanDate': 100, ' ': 50 }}
