...
class OpenLoans extends React.Component {
  ...
  showSingleRenewCallout(loan) {
    const message = (
      <span>
        <FormattedMessage id="ui-users.loans.item.renewed.callout" values={{ title: `<strong>${loan.item.title}</strong>` }} />
      </span>
    );
  ...
  showMultiRenewCallout(loanItems) {
    const message = (
      <span>
        <FormattedMessage id="ui-users.loans.items.renewed.callout" values={{ strongCount: <strong>{loanItems.length}</strong>, count: loanItems.length }} />
  ...
  render() {
    ...
    const subHeading = <p><FormattedMessage id="ui-users.loans.items.renewed.subheading" values={{ strongCount: <strong>{loanItems.length}</strong>, count: loanItems.length }} /></p>;
