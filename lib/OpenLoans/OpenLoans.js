...
class OpenLoans extends React.Component {
  ...
  showCalloutMessage(loanItems, count) {
    ...
    if (count === 1) {
      ...
      message = (
        <span>
          Renewed <strong>{Object.keys(loanItems).length}</strong> loans <Button buttonStyle="link" onClick={() => this.showLoans(loanItems)}><strong> - view details</strong></Button>.
        </span>
      );
    }
  ...
  handleOptionsChange(key, e) {
    ...
    if (key.action && this[key.action]) {
      const renewedLoan = this.renew(key.loan);
      Promise.resolve(renewedLoan).then(
        () => this.showCalloutMessage(key, 1), // 1 represents the number of items checked
      );
    }
  }
  ...
  render() {
    ...
    const label = `The loans for the following ${Object.keys(loanItems).length} instances were successfully renewed`;
    ...
    return (
      <div className={css.root}>
        ...
        <MultiColumnList
          ...
          interactive={false}
        />
        <Modal dismissible closeOnBackgroundClick onClose={this.handleClose} open={this.state.confirming} label={label}>
          {
