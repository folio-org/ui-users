...
class OpenLoans extends React.Component {
  ...
  renewSelected() {
    const renewedLoans = Object.values(this.state.checkedLoans).map(loan => this.renew(loan, true));
    Promise.all(renewedLoans)
      .then(loans => this.afterRenewSelected(loans))
      .catch(error => this.setState({ error }));
  ...
  render() {
    const { sortOrder, sortDirection, allChecked, loanItems, error } = this.state;
