...
class OpenLoans extends React.Component {
  ...
  renderActions(loan) {
    return (
      ...
      >
        <IconButton data-role="toggle" icon="ellipsis" size="small" />
  ...
  renderSubHeader() {
    ...
    return (
      <div className={css.loanActionsBar}>
        <div className={css.loanActionsBarResults}>
          <Label>{this.props.loans.length} Records found</Label>
        </div>
        <div className={css.loanActionsBarButtons}>
          <Button marginBottom0 id="renew-all" disabled={isDisabled} title={stringToolTip} onClick={this.renewSelected}>Renew</Button>
        </div>
      </div>
    );
  }
        ...
        <Modal dismissible closeOnBackgroundClick onClose={this.hideLoansModal} open={this.state.loanModalOpen} label={label}>
          {subHeading}
          {
              loanItems.map((loanItem, index) => (
                <li key={index}>
                  <span>{loanItem.item.title}yo</span>
                </li>))
          }
        </Modal>
