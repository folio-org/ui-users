...
export default class RenderPatronGroupLastUpdated extends React.Component {
  ...
  getGroup() {
    ...
    let group;
    for (const g of inheritedResources.groups.records) {
  ...
  getUser(group) {
    ...
    if (group && group.metadata) {
      const inheritedResources = this.props.additionalFields.lastUpdated.inheritedProps.resources;
      for (const u of inheritedResources.users.records) {
