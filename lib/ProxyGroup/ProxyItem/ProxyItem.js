...
const ProxyItem = ({ record, stripes }) => {
  ...
  const link = (
    <div>
      ...
      {record.proxy && record.proxy.meta && record.proxy.meta.createdDate && <span className={css.creationLabel}>(Relationship created: {stripes.formatDateTime(record.proxy.meta.createdDate)})</span>}
    </div>
  );
