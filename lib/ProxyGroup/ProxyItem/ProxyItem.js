...
const ProxyItem = ({ record, stripes }) => {
  ...
  const link = (
    <div>
      ...
      {record.proxy && record.proxy.metadata && record.proxy.metadata.createdDate && <span className={css.creationLabel}>(Relationship created: {stripes.formatDateTime(record.proxy.metadata.createdDate)})</span>}
    </div>
  );
