...
const ProxyEditItem = ({ name, record, onDelete, intl, stripes }) => {
  ...
  const proxyLink = (
    <div>
      ...
      {record.meta.createdDate && <span className={css.creationLabel}>(<FormattedMessage id="ui-users.proxy.relationshipCreated" />: {stripes.formatDateTime(record.meta.createdDate)})</span>}
    </div>
  );
