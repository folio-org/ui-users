...
class ProxyEditItem extends React.Component {
  ...
  render() {
    const { name, record, onDelete, stripes } = this.props;
    ...
    const proxyLink = (
      <div>
        ...
        {record.proxy && record.proxy.metadata && record.proxy.metadata.createdDate && <span className={css.creationLabel}>(<FormattedMessage id="ui-users.proxy.relationshipCreated" />: {stripes.formatDateTime(record.proxy.metadata.createdDate)})</span>}
      </div>
    );
